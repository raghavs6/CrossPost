...
export default function DashboardPage() {
  ...
  // Fetch the user's posts on mount, then poll so scheduled jobs can move from
  // queued to published/failed without requiring a browser refresh.
  useEffect(() => {
    let isMounted = true

    function refreshPosts(showSpinner: boolean) {
      if (showSpinner) {
        setIsLoading(true)
      }
      listPosts()
    ...
    refreshPosts(true)
    const intervalID = window.setInterval(() => refreshPosts(false), 10000)
    return () => {
      isMounted = false
  ...
  async function handleSubmit() {
    ...
    const linkedPlatforms = new Set(connections.map((connection) => connection.platform))
    const missingConnections = Array.from(connected).filter((platform) => !linkedPlatforms.has(platform))
    if (missingConnections.length > 0) {
      setSubmitError('Connect selected accounts before scheduling.')
      return
    }
